...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	do {
		...
		if (intmask & SDHCI_INT_ERROR) {
			...
			//SD  - CMD19
			if (host->cmd &&
			    (((host->cmd->opcode != 21) && (host->mmc->card) &&
			      (host->mmc->card->type == MMC_TYPE_MMC)) ||
			     ((host->cmd->opcode != 19) && (host->mmc->card) &&
			      (host->mmc->card->type == MMC_TYPE_SD)))) {
		...
		if (bForceDumpCMD) {
			...
			SDHCI_DUMP("%s: [DEBUG]Argument: 0x%08x\n", __func__,
				   sdhci_readl(host, SDHCI_ARGUMENT));
			SDHCI_DUMP(
				"%s: [DEBUG]Resp[0]:   0x%08x | Resp[1]:  0x%08x\n",
				__func__, sdhci_readl(host, SDHCI_RESPONSE),
				sdhci_readl(host, SDHCI_RESPONSE + 4));
			SDHCI_DUMP(
				"%s: [DEBUG]Resp[2]:   0x%08x | Resp[3]:  0x%08x\n",
				__func__, sdhci_readl(host, SDHCI_RESPONSE + 8),
				sdhci_readl(host, SDHCI_RESPONSE + 12));
			if (intmask & SDHCI_INT_ERROR) {
				sdhci_dumpregs(host);
			}
